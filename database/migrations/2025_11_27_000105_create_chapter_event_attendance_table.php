...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('chapter_event_attendance', function (Blueprint $table) {
            ...
            // Make sure this matches users.id exactly
            $table->foreignId('member_id')->constrained('users')->onDelete('cascade');

            // Make sure this matches chapter_events.id exactly
            $table->foreignId('chapter_event_id')->constrained('chapter_events')->onDelete('cascade');

            $table->enum('attendance_status', ['present','absent','interested'])->default('interested');
