...
<div class="max-w-4xl mx-auto mt-10 bg-white shadow-lg rounded-xl p-8">
  ...
  <form action="{{ route('admin.meetups.update', $meetup->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
    ...
    <div>
      ...
      <input type="text" name="title" value="{{ old('title', $meetup->title) }}" class="w-full border rounded px-3 py-2" required>
    </div>

    {{-- Short Description --}}
    <div>
      <label class="block text-gray-700 font-medium">Short Description</label>
      <textarea name="short_description" rows="2" class="w-full border rounded px-3 py-2">{{ old('short_description', $meetup->short_description) }}</textarea>
    </div>

    {{-- Description --}}
    <div>
      <label class="block text-gray-700 font-medium">Description</label>
      <textarea name="description" rows="6" class="w-full border rounded px-3 py-2">{{ old('description', $meetup->description) }}</textarea>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input type="date" name="event_date" value="{{ old('event_date', $meetup->event_date) }}" class="w-full border rounded px-3 py-2">
      </div>

      <div>
        <label class="block text-gray-700 font-medium">City</label>
        <input type="text" name="city" value="{{ old('city', $meetup->city) }}" class="w-full border rounded px-3 py-2">
      </div>

      <div>
        <label class="block text-gray-700 font-medium">Venue Name</label>
        <input type="text" name="venue_name" value="{{ old('venue_name', $meetup->venue_name) }}" class="w-full border rounded px-3 py-2">
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input type="time" name="start_time" value="{{ old('start_time', $meetup->start_time) }}" class="w-full border rounded px-3 py-2">
      </div>

      <div>
        <label class="block text-gray-700 font-medium">End Time</label>
        <input type="time" name="end_time" value="{{ old('end_time', $meetup->end_time) }}" class="w-full border rounded px-3 py-2">
      </div>
    </div>
