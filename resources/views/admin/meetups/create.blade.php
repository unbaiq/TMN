...
<div class="max-w-4xl mx-auto mt-10 bg-white shadow-lg rounded-xl p-8">
  <h2 class="text-2xl font-semibold text-gray-800 mb-6">Add New Meetup</h2>

  <form action="{{ route('admin.meetups.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
    @csrf
    ...
    <div>
      ...
      <textarea name="description" rows="6" class="w-full bg-white border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400 outline-none px-3 py-2"></textarea>
    </div>

    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="block text-gray-700 font-medium">Event Date</label>
        <input type="date" name="event_date" class="w-full bg-white border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400 outline-none px-3 py-2">
      </div>

      <div>
        <label class="block text-gray-700 font-medium">City</label>
        <input type="text" name="city" class="w-full bg-white border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400 outline-none px-3 py-2">
      </div>
    </div>

    <div>
      <label class="block text-gray-700 font-medium">Venue Name</label>
      <input type="text" name="venue_name" class="w-full bg-white border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400 outline-none px-3 py-2">
    </div>
