    ...
    <tbody>
      @forelse($sponsors as $sponsor)
        <tr class="border-b hover:bg-gray-50">
          <td class="px-4 py-3 font-medium">{{ $sponsor->name }}</td>
          <td class="px-4 py-3">{{ $sponsor->company_name ?? '—' }}</td>
          <td class="px-4 py-3 capitalize">{{ $sponsor->sponsor_type }}</td>
          <td class="px-4 py-3 capitalize">{{ $sponsor->sponsorship_level }}</td>
