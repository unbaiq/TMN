...
<div class="max-w-4xl mx-auto mt-10 bg-white shadow-lg rounded-xl p-8">
    ...
    <form action="{{ route('admin.chapters.update', $chapter->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <input name="name" value="{{ old('name', $chapter->name) }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-gray-700 font-medium">City</label>
            <input name="city" value="{{ old('city', $chapter->city) }}" class="w-full border rounded px-3 py-2">
        </div>

        <div>
            <label class="block text-gray-700 font-medium">Pincode</label>
            <input name="pincode" value="{{ old('pincode', $chapter->pincode) }}" class="w-full border rounded px-3 py-2">
        </div>

        <div>
            <label class="block text-gray-700 font-medium">Capacity</label>
            <input name="capacity_no" type="number" min="0" value="{{ old('capacity_no', $chapter->capacity_no) }}" class="w-full border rounded px-3 py-2">
        </div>
