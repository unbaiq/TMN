...
<div class="max-w-7xl mx-auto mt-10 bg-white shadow-xl rounded-2xl p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        ...
        <a href="{{ route('admin.chapters.create') }}"
           class="bg-gradient-to-r from-[#CF2031] to-[#b81b2a]
                  hover:from-[#b81b2a] hover:to-[#991826]
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs font-semibold border-b">
                <tr>
                    ...
                    <th class="py-3 px-4 text-left">Chapter</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($chapters as $chapter)
                    @php
                        $current = $chapter->members_count ?? 0;
                        $limit   = $chapter->capacity_no ?? 0;
                        $isFull  = $limit > 0 && $current >= $limit;
                        $percent = $limit > 0 ? min(100, ($current / max(1, $limit)) * 100) : 0;
                    @endphp
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-3 px-4">
                            @if($chapter->logo)
                                <img src="{{ asset('storage/'.$chapter->logo) }}"
                                     class="h-10 w-10 object-cover rounded-full border shadow-sm">
                            @else
                        ...
                        <td class="py-3 px-4 text-gray-700">
                            {{ collect([$chapter->city, $chapter->state, $chapter->country])->filter()->implode(', ') ?: '—' }}
                        </td>
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-2">
                                ...
                                @if($limit)
                                    <div class="h-2 w-28 bg-gray-200 rounded-full overflow-hidden">
                            ...
                            @if($isFull)
                                <p class="text-xs text-red-500 mt-1">Capacity reached</p>
                            @endif
                        </td>
                        ...
                        <td class="py-3 px-4">
                            @if($chapter->is_active)
                                <span class="inline-flex items-center gap-1 px-2 py-1
                                             rounded-full text-xs bg-green-100 text-green-700 font-medium">
                                    <i data-feather="check-circle" class="w-3 h-3"></i>
                                    Active
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1 px-2 py-1
                                             rounded-full text-xs bg-gray-100 text-gray-600 font-medium">
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex flex-wrap justify-end gap-2">
                                ...
                                <a href="{{ route('admin.chapters.members', $chapter->id) }}"
                                   class="action-btn bg-indigo-50 text-indigo-700 hover:bg-indigo-100">
                                ...
                                <form action="{{ route('admin.chapters.destroy', $chapter->id) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button class="action-btn bg-red-50 text-red-700 hover:bg-red-100">
