...
(function(){
  ...
  // NEW helper: next sequential numeric id (keeps codes readable)
  function getNextSequentialId(){
    if (!chapters.length) return 1;
    return Math.max(...chapters.map(c => Number(c.id) || 0)) + 1;
  }
  ...
  if (!chapterMeta) {
    ...
    chapterMeta.id = 'chapterMeta';
    chapterMeta.className = 'text-xs text-gray-500 mb-2';
    chapterModal.querySelector('form').insertAdjacentElement('beforebegin', chapterMeta);
  }
    ...
    card.innerHTML = `
      ...
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <div class="mt-1 text-xs text-gray-500"> <strong>ID</strong>: <span class="font-mono">${escapeHtml(String(ch.id))}</span> • <strong>Code</strong>: <span class="font-mono">${code}</span></div>
          <p class="text-sm text-gray-500 mt-2">${escapeHtml(ch.city||'')}</p>
  ...
  function renderChaptersFiltered(){
    ...
    const filtered = chapters.filter(c => !q || c.name.toLowerCase().includes(q) || (c.city||'').toLowerCase().includes(q) || (c.slug||'').toLowerCase().includes(q) || (c.chapter_code||'').toLowerCase().includes(q) );
  ...
  chapterForm.addEventListener('submit', (e) => {
    ...
    if (existing && existing.chapter_code) {
      ...
      payload.chapter_code = padChapterCodeFromId(newId);
    }
