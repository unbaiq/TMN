...
<div id="inv-page" class="max-w-7xl mx-auto px-4 py-8">
  <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-4">
    <div>
      <h2 class="text-lg md:text-xl font-semibold text-gray-800">Investors</h2>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
      <div class="flex items-center gap-2 w-full sm:w-auto">
        <input id="inv-globalSearch" type="search"
               placeholder="Search by member, category, interest..."
               class="px-3 py-2 border rounded-md text-sm w-full sm:w-72 focus:outline-none focus:ring-2 focus:ring-red-200" />
        <button id="inv-searchBtn" class="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm">Search</button>
      </div>

      <div class="flex items-center gap-2 ml-auto">
        <button id="inv-exportCsvBtn" class="px-3 py-2 bg-white border rounded text-sm">Export</button>
        <button id="inv-openAddBtn" class="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm">Add Investor</button>
      </div>
    </div>
  </div>
...
<div id="inv-viewModal" class="fixed inset-0 z-50 hidden items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="inv-view-title">
  ...
  <div class="relative max-w-2xl w-full mx-4">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b">
        <h3 id="inv-view-title" class="text-lg font-semibold">Investor Details</h3>
        <button id="inv-viewModalCloseX" class="text-gray-500 hover:text-gray-700" aria-label="Close">✕</button>
      </div>

      <div id="inv-viewModalContent" class="p-6 text-sm"></div>
      ...
      <div class="p-4 border-t flex justify-between items-center gap-3">
        ...
        <div class="flex gap-2">
          <button id="inv-viewModalCloseBtn" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">Close</button>
        </div>
      </div>
    </div>
  </div>
...
<!-- ADD INVESTOR MODAL -->
<div id="inv-addModal" class="fixed inset-0 z-50 hidden items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="inv-add-title">
  <div id="inv-addOverlay" class="absolute inset-0 bg-black/40"></div>
  <div class="relative max-w-xl w-full mx-4">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b">
        <h3 id="inv-add-title" class="text-lg font-semibold">Add Investor</h3>
        <button id="inv-addModalCloseX" class="text-gray-500 hover:text-gray-700" aria-label="Close">✕</button>
      </div>
      ...
      <form id="inv-addForm" class="p-6 space-y-3">
        ...
        <div class="flex items-center gap-2 justify-end pt-2">
          ...
          <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">Add Investor</button>
        </div>
      </form>
...
<!-- SCRIPTS -->
<script>
/* INVESTORS PAGE — improved responsive UI + behavior */
document.addEventListener('DOMContentLoaded', () => {
  const isAdmin = false; // flip when testing admin actions
  ...
  let investors = [
    ...
    { id: 3, member_id: 112, member_name: 'Rahul Mehta', investment_amount: 100000.00, member_notes: '', interested_categories: 'FMCG', startup_interest: 'Early-stage', status: 'rejected', invests_in_type: 'company', invests_in_name: 'Rahul Foods', contact: 'rahul@example.com', documents: [{name:'KYC.pdf', url:'https://example.com/files/kyc.pdf', size:'120 KB'}], created_at:'2025-06-20' }
  ];
      ...
      <div class="md:hidden px-4 py-4 bg-white">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            ...
            <div class="text-xs text-gray-400 mt-2 line-clamp-2">${esc(it.member_notes || '')}</div>
          </div>
  ...
  exportCsvBtn.addEventListener('click', () => {
    const rows = filtered.map(r => [ r.member_name || ('Member #' + r.member_id), r.member_id, r.investment_amount, r.invests_in_type || '', r.invests_in_name || '', r.interested_categories || '', r.startup_interest || '', r.member_notes || '', r.status ]);
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (item.documents && item.documents.length) {
      ...
      html += `<div class="mt-2"><button id="inv-downloadAllDocsBtn" class="px-3 py-1 bg-red-600 text-white rounded text-sm">Download All</button></div></div>`;
    }
    ...
    if (isAdmin) {
      ...
      verifyBtn.addEventListener('click', () => { item.status = 'verified'; item.verified_by = 1; render(); });
      ...
      rejectBtn.addEventListener('click', () => { const r = prompt('Rejection reason (optional):','Insufficient docs'); item.status='rejected'; item.rejection_reason=r; render(); });
  ...
  function closeViewModal(){ viewModal.classList.add('hidden'); viewModal.style.display = 'none'; document.body.style.overflow = ''; }
  ...
  // add modal
  function openAddModal(){ addModal.classList.remove('hidden'); addModal.style.display = 'flex'; document.body.style.overflow = 'hidden'; }
  function closeAddModal(){ addModal.classList.add('hidden'); addModal.style.display = 'none'; document.body.style.overflow = ''; }
  ...
  // refresh icons if feather available
