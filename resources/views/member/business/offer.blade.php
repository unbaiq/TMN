...
<div class="p-4 sm:p-6 lg:p-10 space-y-6">
  ...
  <div class="bg-white p-3 rounded-lg shadow-sm">
    <button id="filtersToggle" aria-expanded="true"
      ...
      <svg id="filtersToggleIcon" class="w-5 h-5 text-gray-600 transform transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
    </button>

    <div id="filtersPanel" class="mt-3 md:mt-0 grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
      <div>
        <label class="text-xs text-gray-600">Search</label>
        <input id="searchInput" type="search" placeholder="Service, receiver..."
               class="mt-1 block w-full px-3 py-2 rounded-md border border-slate-200 focus:ring-2 focus:ring-red-200 focus:border-red-400">
      </div>
      ...
      <div>
        ...
        <select id="statusFilter" class="mt-1 block w-full px-3 py-2 rounded-md border border-slate-200 focus:ring-2 focus:ring-red-200 focus:border-red-400">
      ...
      <div class="flex flex-col items-end gap-2">
        ...
        <div class="flex gap-2 mt-1 w-full sm:w-auto">
          <button id="clearFilters" class="flex-1 sm:flex-none px-3 py-2 bg-white border rounded-md text-sm">Clear</button>
          <button id="applyFilters" class="flex-1 sm:flex-none px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm">Apply</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="bg-white rounded-lg shadow-sm overflow-hidden">
    ...
    <div class="px-4 py-3 border-t bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        <button id="firstPage" class="px-3 py-1 bg-white border rounded-md disabled:opacity-50" aria-label="First page">First</button>
        <button id="prevPage" class="px-3 py-1 bg-white border rounded-md disabled:opacity-50" aria-label="Previous page">Prev</button>
        <button id="nextPage" class="px-3 py-1 bg-white border rounded-md disabled:opacity-50" aria-label="Next page">Next</button>
      </div>
    </div>
  </div>
...
{{-- Improved script: debounce, filters accordion, mobile-friendly UI actions --}}
<script>
document.addEventListener('DOMContentLoaded', () => {

  // ---- small helpers ----
  const $ = id => document.getElementById(id);
  const escHtml = v => v==null ? '' : String(v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');

  // ----- sample data -----
  window.services = window.services || [
    { id:1, service_name:"Technical Audit", description:"Audit of website performance and SEO improvements.", receiver_name:"Rahul Mehta", receiver_member_id:112, status:"pending", taker_request:true, taker_contact:{ name:'Rahul Mehta', member_id:112, phone:'+91-9876543210', email:'rahul@example.com' }, taker_documents:[{name:'audit-log.txt',url:'#',size:'45 KB'}] },
    { id:2, service_name:"Website Audit", description:"Taker uploaded logs and screenshots indicating issues.", receiver_name:"Anita Singh", receiver_member_id:401, status:"accepted", taker_request:false },
    { id:3, service_name:"Logo Design", description:"Created logo and brand assets.", receiver_name:"Vikram", receiver_member_id:77, status:"rejected", taker_request:false }
  ];
  ...
  if (filtersToggle && filtersPanel) {
    ...
    // On desktop, filtersPanel should always be visible
    ...
    if (s.taker_request) {
      return `
        <div class="hidden md:flex items-center justify-end gap-2">
          <button class="acceptBtn inline-block px-3 py-1 rounded bg-green-600 text-white text-xs" data-id="${s.id}">Accept</button>
          <button class="rejectBtn inline-block px-3 py-1 rounded bg-red-600 text-white text-xs" data-id="${s.id}">Reject</button>
          <button class="viewBtn inline-block text-sm text-blue-600" data-id="${s.id}">View</button>
        </div>
      `;
    }
    return `<div class="hidden md:flex items-center justify-end"><button class="viewBtn text-sm text-blue-600" data-id="${s.id}">View</button></div>`;
  }
  ...
  function buildRowHtml(s){
    ...
    return `
      <div class="hidden md:grid grid-cols-12 gap-4 px-4 py-4 items-center text-sm">
        ...
        <div class="col-span-1 text-center">${statusBadgeHtml(s.status)}</div>
        <div class="col-span-1">${actionButtonsHtml(s)}</div>
      </div>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between gap-2">
              ...
              <div>${statusBadgeHtml(s.status)}</div>
            </div>
            <div class="text-xs text-gray-500 mt-1 line-clamp-2">${escHtml(s.description || '')}</div>
            <div class="text-xs text-gray-400 mt-2">Receiver: ${escHtml(s.receiver_name || '—')} • ${escHtml(s.receiver_member_id || '—')}</div>

            <div class="mt-3 space-y-2">
              ${s.taker_request ? `
                <div class="flex gap-2">
                  <button class="acceptBtnMobile flex-1 px-3 py-2 rounded bg-green-600 text-white text-sm" data-id="${s.id}">Accept</button>
                  <button class="rejectBtnMobile flex-1 px-3 py-2 rounded bg-red-600 text-white text-sm" data-id="${s.id}">Reject</button>
                </div>` : ''}
              <div>
  ...
  function render(){
    ...
    if (list.length === 0) {
      ...
      list.forEach(s => rowsContainer.insertAdjacentHTML('beforeend', buildRowHtml(s)));
    }
  ...
  function handleAccept(id) {
    ...
    if (updateStatusForId(id, 'accepted')) {
      render();
  ...
  function handleReject(id) {
    ...
    if (updateStatusForId(id, 'rejected')) {
      const s = services.find(x => String(x.id) === String(id));
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (acceptBtn) { handleAccept(acceptBtn.dataset.id); return; }

    const rejectBtn = ev.target.closest && ev.target.closest('.rejectBtn');
    if (rejectBtn) { handleReject(rejectBtn.dataset.id); return; }

    const acceptBtnMobile = ev.target.closest && ev.target.closest('.acceptBtnMobile');
    if (acceptBtnMobile) { handleAccept(acceptBtnMobile.dataset.id); return; }

    const rejectBtnMobile = ev.target.closest && ev.target.closest('.rejectBtnMobile');
    if (rejectBtnMobile) { handleReject(rejectBtnMobile.dataset.id); return; }
  });
  ...
  function showServiceModalFor(s){
    ...
    if (s.taker_documents && s.taker_documents.length) {
      let docsHtml = `<hr class="my-4" /><div class="text-sm"><div class="font-medium mb-2">Taker documents</div><div class="space-y-2">`;
      s.taker_documents.forEach(d => {
        docsHtml += `<div class="flex items-center justify-between bg-gray-50 p-2 rounded">
    ...
    modalContent.querySelectorAll('.acceptBtnModal').forEach(b => b.addEventListener('click', () => { handleAccept(b.dataset.id); hideServiceModal(); }));
    modalContent.querySelectorAll('.rejectBtnModal').forEach(b => b.addEventListener('click', () => { handleReject(b.dataset.id); hideServiceModal(); }));
  ...
  function hideServiceModal(){ if (!serviceModal) return; serviceModal.classList.add('hidden'); serviceModal.style.display = 'none'; document.body.style.overflow = ''; if (modalContent) delete modalContent.dataset.currentId; }
