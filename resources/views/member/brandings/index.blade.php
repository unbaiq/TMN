...
<div class="max-w-8xl mx-auto px-4 py-4 space-y-6">
    ...
    <div class="bg-gradient-to-r from-red-700 via-red-600 to-red-500 text-white rounded-2xl px-8 py-6 flex justify-between items-center shadow-lg">
        ...
        <a href="{{ route('member.brandings.create') }}"
           class="bg-white text-red-600 px-4 py-2 rounded-lg font-medium shadow hover:bg-gray-100 transition">
            <i data-feather="plus" class="inline w-4 h-4 mr-1"></i> Add Branding
        </a>
    </div>
    ...
    <form method="GET" class="bg-white border border-gray-200 shadow rounded-xl p-4 flex flex-wrap gap-4 items-end">
        <input type="text" name="search" value="{{ request('search') }}"
               placeholder="Search title, platform or publication..."
        ...
        <select name="branding_type"
            ...
            @foreach([
                ...
                <option value="{{ $key }}" {{ request('branding_type') == $key ? 'selected' : '' }}>{{ $label }}</option>
            @endforeach
        </select>
    ...
    <div class="bg-white border border-gray-200 shadow rounded-2xl overflow-hidden">
        @if(isset($brandings) && $brandings->count())
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                            @if($branding->thumbnail_url)
                                <img src="{{ $branding->thumbnail_url }}" class="w-10 h-10 rounded object-cover border" alt="">
                            @endif
                            <div>
                    ...
                    <td class="px-4 py-3 capitalize">{{ str_replace('_',' ',$branding->branding_type) }}</td>
                    <td class="px-4 py-3">
                        <div class="text-gray-700">{{ $branding->media_platform ?? '—' }}</div>
                        @if($branding->media_link)
                            <a href="{{ $branding->media_link }}" target="_blank" class="text-xs text-blue-600 underline">
                                View Link
                            </a>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 text-right space-x-3">
                        <a href="{{ route('member.brandings.show', $branding) }}" class="text-blue-600 hover:underline">View</a>
                        <a href="{{ route('member.brandings.edit', $branding) }}" class="text-green-600 hover:underline">Edit</a>
                        <form action="{{ route('member.brandings.destroy', $branding) }}" method="POST" class="inline">
                            @csrf @method('DELETE')
                            <button onclick="return confirm('Delete this branding record?')" class="text-red-600 hover:underline">
                                Delete
                            </button>
                        </form>
                    </td>
        ...
        @else
            <div class="p-10 text-center text-gray-500">No branding activities found.</div>
        @endif
    </div>
</div>
