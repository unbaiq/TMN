...
<div class="max-w-8xl mx-auto px-4 py-4 space-y-6">
    ...
    {{-- ==== FILTER + SEARCH ==== --}}
    <div class="bg-white border border-gray-200 shadow-md rounded-xl p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center justify-start gap-1.5 md:gap-2 bg-gray-50 border border-gray-200 rounded-full px-2 py-1.5 shadow-sm">
    <button
        ...
        @forelse ($events as $event)
            ...
            <div class="event-card bg-white border border-gray-200 p-6 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 hover:border-red-400"
                ...
                <div class="text-sm text-gray-600 space-y-1">
                    <p class="flex items-center gap-2"><i data-feather="map-pin" class="w-4"></i> {{ $event->venue_name ?? '—' }}</p>
                    <p class="flex items-center gap-2"><i data-feather="calendar" class="w-4"></i> {{ $event->formatted_date }}</p>
...
{{-- ==== MODAL ==== --}}
<div id="eventModal" class="hidden fixed inset-0 z-50 bg-black/60 flex items-center justify-center backdrop-blur-sm transition-opacity duration-300">
    <div class="modal-card bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden border border-red-100 transform scale-90 opacity-0 transition-all duration-300 relative">
        <button onclick="closeEventModal()" class="absolute top-4 right-4 bg-white/90 hover:bg-red-600 hover:text-white text-gray-700 p-2 rounded-full shadow-md transition">
        ...
        <div class="p-6 space-y-5">
            <div>
                ...
                <p id="modalStatus" class="inline-block mt-1 text-xs px-3 py-1 rounded-full font-semibold bg-red-50 text-red-600"></p>
            </div>
            ...
            <div class="border-t border-gray-200 pt-4 space-y-3 text-sm">
                ...
                <p id="modalLinkWrapper" class="hidden"><strong>🔗 Meeting Link:</strong> <a id="modalLink" href="#" target="_blank" class="text-red-600 underline"></a></p>
                <p id="modalPassWrapper" class="hidden"><strong>🔑 Meeting Password:</strong> <span id="modalPass"></span></p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-3">
                <button onclick="confirmAttendance()" class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg shadow-md flex items-center justify-center gap-2 transition">
                    <i data-feather="check"></i> Confirm Attendance
                </button>
                <button onclick="closeEventModal()" class="flex-1 bg-gray-800 hover:bg-gray-900 text-white py-2 rounded-lg shadow-md flex items-center justify-center gap-2 transition">
...
.modal-card { transition: all 0.25s ease-in-out; }
</style>
@endsection
